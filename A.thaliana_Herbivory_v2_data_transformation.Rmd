...
(1) Explore data via plotting plant traits (biomass, number of trichome, absolute leaf area eaten by herbivores) and herbivore/caterpillar performance (relative growth rate/RGR) combining with spatial data.

(2) Perform statistical analysis on how herbivory affects plant traits and how plant traits and herbivore performance are influenced by urbanisation.
...
# 1. Outliers
par(mfrow = c(3,3))
dp.RGR <- dotchart(cater.RGR$RGR, main = "RGR")
dp.tric <- dotchart(trichome$trichome, main = "trichome")
dp.AG <- dotchart(plant.biomass$BG.biomass, main = "root biomass")
dp.BG <- dotchart(plant.biomass$AG.biomass, main = "shoot biomass")
dp.aread <- dotchart(area.H$area.damaged, main = "leaf area damaged")
dp.areaH <- dotchart(area.H$area.total, main = "total leaf area in herbivory group")
dp.areaC <- dotchart(area.C$area.total, main = "total leaf area in control group")

```

Results:
(1) Two observations with high "trichome" values, one observation with high and isolated "root biomass" value, several observations with high "leaf area damaged" values and two observations with high "total leaf area in control group".

(2) We see large values in three leaf area variables, it is better therefore to apply the transformation on them before continuing the analysis. 


# **3.2 Collinearity and correlations of variables**

Only within herbivory treatment, correlations are calculated among relative growth rate (RGR) of caterpillars and number of trichomes, absolute leaf area damaged by caterpillars.

```{r}
# apply log10() transformation on "trichome", "absolute leaf area damaged" and "total leaf area"
trichome$trichome <- log10(trichome$trichome)
area.C$area.total <- log10(area.C$area.total)
area.H$area.total <- log10(area.H$area.total)
area.H$area.damaged <- log10(area.H$area.damaged)
...
cor.herbivory <- select(herbivory, RGR, trichome, area.damaged, area.total)
...
# **3.3.9 Compute one-way ANOVA test**
...
Model: variables ~ urbanisation(continuous)
...
# **4.3.2. Based on average data on trichome of each mother plant**
...
# **6.1 Effect of herbivory and urbanisation(continuous) on average root biomass among genotypes**
...
# **7. Differences of biomass along with urbanisation(continuous)**
